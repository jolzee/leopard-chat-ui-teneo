...
const audioFile = require("@/assets/notification.mp3");
...
const browserHandledSession = null;
...
function storeSetup(vuetify) {
	...
	store = new Vuex.Store({
		...
		getters: {
			...
			getPromptPollingIntervalInMilliseconds(state) {
				if (
					...
					return parseInt(state.activeSolution.promptTriggers.pollSeconds, 10) * 1000;
				}
			...
			uploadConfig(_state, getters) {
				const item = getters.lastReplyItem;
			...
			socialAuthEnabled: state => !!state.auth.firebase,
			lastReplyItem: state => {
			...
			askingForPassword(_state, getters) {
				...
				if (item && item.teneoResponse) {
					const inputType = decodeURIComponent(item.teneoResponse.extraData.inputType);
			...
			inputHelpText(_state, getters) {
				...
				let inputHelpText;
				if (item && item.teneoResponse) {
					const helpText = decodeURIComponent(item.teneoResponse.extraData.inputHelpText);
			...
			itemInputMask(_state, getters) {
				...
				let itemInputMask;
				if (item && item.teneoResponse) {
					const mask = decodeURIComponent(item.teneoResponse.extraData.inputMask);
					if (mask !== "undefined") {
						itemInputMask = mask;
					}
				}
			...
			isVideoFile: _state => url => {
				...
				const result = match ? match[0].substring(1, match[0].length) : false;
				logger.debug(result);
				return result;
			},
			...
			textColor: state => themeColorName => {
				if (!vuetify.framework.theme.isDark) {
					const hexColor = state.ui.theme[themeColorName];
			...
			itemExtensionsModal: (_state, getters) => item => {
				const extensions = getters.itemExtensions(item);
			...
			itemExtensions: _state => item => {
				const actions = [];
				if (item && item.teneoResponse) {
					const hasExtensions = Object.keys(item.teneoResponse.extraData).some(
						function checkForExtensions(key) {
							return String(key).startsWith("extensions");
						}
					);
					if (hasExtensions) {
						...
						Object.keys(item.teneoResponse.extraData)
							.sort()
							.forEach(function placeExtensionsInSortedList(key) {
						...
						try {
							Object.entries(ordered).forEach(entry => {
								let value = entry[1];
								const key = entry[0];
								if (String(key).startsWith("extensions")) {
									try {
			...
			ctxParameters(state) {
				const ctxParams = state.connection.ctxParameters;
				if (ctxParams) {
					const queryParams = Object.keys(ctxParams)
			...
			hasModal: (_state, getters) => item => {
				...
				extensions.forEach(extension => {
					if (
						...
						!String(extension.name).startsWith("displayRouterCheckList")
					) {
						hasModal = true;
					}
				});
			...
			youTubeVideoId: (_state, getters) => extension => {
				if (extension && extension.name === "displayVideo") {
					...
					const videoId = getters.youTubeIdFromUrl(url);
					if (videoId) {
						return videoId;
					}
				}
			...
			mapInfo: (_state, getters) => extension => {
				if (extension && extension.name === "displayMap") {
					const { address } = extension.parameters;
					if (address) {
						return {
							address
						};
			...
			dialogs(state) {
				...
				if (!config.USE_SESSION_STORAGE && state.conversation.dialog.length === 0) {
					...
					if (!lastInteractionTime) {
						...
						let diff = (now.getTime() - lastInteractionTime) / 1000;
						diff /= 60;
		...
		mutations: {
			...
			SHOW_SIMPLE_MESSAGE_IN_CHAT(state, msgConfig) {
				...
				Object.assign(miscMessage, msgConfig);
			...
			UPDATE_CHAT_WINDOW_AND_STORAGE(state, payload) {
				...
				if (
					...
					(Object.keys(payload.response.teneoResponse.extraData).some(function checkForExtensions(
						key
					) {
						return String(key).startsWith("extensions");
					}) ||
			...
			REMOVE_FORM_CONFIG(state, itemId) {
				const foundHistory = state.conversation.dialogHistory.find(function locateItemInHistory(
					item
				) {
					return item.id === itemId;
				});

				const found = state.conversation.dialog.find(function locateItemInDialog(item) {
					return item.id === itemId;
				});
		...
		actions: {
			shouldBackupSolutionsConfig() {
				return new Promise(resolve => {
					...
					const lastSolutionBackupDate = localStorage.getItem(`${STORAGE_KEY}lastBackupDate`);

					if (lastSolutionBackupDate) {
						const pastDate = dayjs(lastSolutionBackupDate);
			...
			liveChatAddCannedResponse(context, liveChatConfiguration) {
				// config.data.token = context.getters.liveChatApiToken;
				logger.debug(`Adding LiveChat Canned Response`, liveChatConfiguration);

				superagent
					.post(`${window.leopardConfig.liveChat.agentAssistServerUrl}/can`)
					.accept("json")
					.type("json")
					.send(liveChatConfiguration)
					.then(res => {
			...
			openChatWindow(context, mustLogin = true) {
				...
				if (!context.getters.isChatOpen) {
					...
					const isChatUiFloating = context.getters.float;
					setTimeout(function handleButtonsAndFrames() {
						...
						if (!context.getters.embed && !context.getters.overlayChat && siteFrame) {
							setTimeout(function contractIframe() {
					...
					if (mustLogin) {
						context
							...
							.then(() => {
								...
								setTimeout(function showChatButton() {
								...
								}, 1500); // only show the chat button after a successful login
							})
							.catch(err => {
			...
			sendFeedback(context, feedback) {
				...
				const queryObj = queryParamStringAsObject(queryParams);
			...
			setUserInformation({ commit, getters }) {
				...
				if (getters.firebase) {
					...
					getters.firebase.auth().onAuthStateChanged(function saveUser(user) {
						if (user) {
							commit("USER_INFO", { user }); // user is still signed in
						}
					});
				} else {
					let retyCount = 0;
					const checkExist = setInterval(function keepLooking() {
						retyCount += 1;
			...
			loginSocial({ commit, getters }, socialProvider) {
				return new Promise((resolve, reject) => {
					...
					getters.firebase
						...
						.catch(function handleErorr(error) {
							// Handle Errors here.
			...
			registerUserWithUsernameEmailPassword({ commit, getters }, registrationInfo) {
				return new Promise((resolve, reject) => {
					...
					getters.firebase
						...
						.then(user => {
							const { currentUser } = getters.firebase.auth();
					...
					TIE.sendInput(config.TENEO_URL, browserHandledSession, queryObj)
						.then(json => {
							...
							if ("numActiveFlows" in json.responseData.extraData) {
								const numActiveFlows = parseInt(json.responseData.extraData.numActiveFlows, 10);
								if (numActiveFlows > 0) {
									...
									if (
										...
										const interval = setInterval(function sendPromptTrigger() {
							...
							if (
								Object.keys(json.responseData.extraData).some(function checkForExtensions(key) {
									return String(key).startsWith("extensions");
								}) ||
			...
			async sendUserInput(context, params = "") {
				...
				if (currentUserInput) {
					try {
						...
						logger.debug(`Could not play audio file`);
					}
				}
					...
					TIE.sendInput(config.TENEO_URL, browserHandledSession, queryObj)
						.then(json => {
							...
							if ("numActiveFlows" in json.responseData.extraData) {
								...
								if (numActiveFlows > 0) {
									...
									if (
										...
										logger.debug("Start up Prompt Trigger Polling");
										const interval = setInterval(function triggerSendOfPromptTrigger() {
											context.dispatch("sendUserInput", "&command=prompt");
										}, context.getters.getPromptPollingIntervalInMilliseconds);
										context.commit("SET_PROMPT_TRIGGER_INTERVAL", interval);
							...
							if (json.responseData.extraData.offerFeedbackForm) {
								...
								try {
									...
									try {
										...
										feedbackConfig = json.responseData.extraData.offerFeedbackForm;
									}
								}
							...
							if ("script" in json.responseData.extraData) {
								const theScript = decodeURIComponent(json.responseData.extraData.script);
							...
							if (
								"inputType" in json.responseData.extraData &&
								String(json.responseData.extraData.inputType).startsWith("location")
							) {
								config
									.getLocator()
									.then(function handleLocationReceive(position) {
										...
										if (json.responseData.extraData.inputType === "locationLatLong") {
											...
											const locationRequestType = json.responseData.extraData.inputType;
											superagent
												.get(
												...
												.then(res => {
													...
													let queryParam = `&${window.locationRequestType}=`;
							...
							if (response.teneoResponse) {
								...
								if (response.teneoResponse.extraData.chatTitle) {
									const chatTitle = decodeURIComponent(response.teneoResponse.extraData.chatTitle);
								...
								const langInput = decodeURIComponent(response.teneoResponse.extraData.langinput);
								const langEngineUrl = decodeURIComponent(
									response.teneoResponse.extraData.langengineurl
								);
								const lang = decodeURIComponent(response.teneoResponse.extraData.lang);
								const langurl = decodeURIComponent(response.teneoResponse.extraData.langurl);
	...
	Object.keys(TRANSLATIONS).forEach(function populateTranslations(key) {
	...
	try {
		...
		if (event.data) {
			...
			if ("leopardState" in messageObject && messageObject.leopardState === "closed") {
				...
				setTimeout(function handleSessionKill() {
			...
			if ("height" in messageObject && "width" in messageObject) {
				...
				logger.debug(
					`handleMessageFromParent: height: ${messageObject.height} width: ${messageObject.width}`
				);
			} else {
