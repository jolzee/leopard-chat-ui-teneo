<template>
	...
	<div v-if="item.type === 'reply'" :class="itemIndexInDialog === dialog.length - 1 ? 'pb-3' : ''">
		<v-row v-if="itemText !== '<span>'" justify="start" no-gutters class="pr-3 pl-1 pt-2">
			...
			<v-col class="text-left" :class="!showChatIcons || $vuetify.breakpoint.smAndDown ? 'ml-2' : ''">
				<v-card
				...
				<v-col cols="12">
					...
					>
						<template v-slot:default>
							<thead>
								<tr>
									<th v-for="header in extension.headers" :key="header + uuid" class="text-left">{{ header }}</th>
								</tr>
							</thead>
							<tbody>
		...
		<v-row v-if="!completedForm && hasForm()" no-gutters class="mr-4">
			<v-col cols="12" class="text-right mb-2">
				...
				<v-btn
					...
					@click="showForm()"
				>
					<v-icon left class="teneo-icon" style="opacity: 0.7 !important;">mdi-file-document-edit-outline</v-icon>
					{{
