					...
					<v-row align="start" justify="start">
						<v-col cols="12" class="pa-2 pb-0">
							<v-container fluid class="py-0">
								<v-row>
									<v-col v-if="hasSolutions" cols="12" class="pb-0">
										<v-autocomplete
							...
							>
								<v-card>
									<v-system-bar
										...
										:class="{
											'grab-cursor': !embed && !$vuetify.breakpoint.mdAndDown
										}"
										dark
									>
									...
									<v-app-bar :color="`primary ${textColor('primary')}`" dense style="max-height:48px">
							...
							<Dialog :show="audit.show" :title="audit.title" width="1000px" @close="closeAuditDialog">
								<v-simple-table>
							...
							<ConfigAddEditSolution
								...
								:current-mode-edit="currentModeEdit"
								:config="config"
								:selected-solution="solution"
								@result="closeAddNewSolutionDialog($event)"
							></ConfigAddEditSolution>
						</v-col>
					</v-row>
				</v-container>
...
export default {
	...
	computed: {
		...
		getDefaultSolutionName() {
			const defaultSolution = this.config.solutions.find(
		...
		getActiveSolutionDeepLink() {
			...
			if (this.selectedSolution) {
				return `${window.location.protocol}//${window.location.host}${window.location.pathname}?dl=${this.selectedSolution.deepLink}`;
			}
		...
		getActiveSolutionDeepLinkMobile() {
			sessionStorage.removeItem(`${STORAGE_KEY}teneo-chat-history`);
			if (this.selectedSolution) {
				return `${window.location.protocol}//${window.location.host}${window.location.pathname}mobile.html?dl=${this.selectedSolution.deepLink}`;
			}
			return `${window.location.protocol}//${window.location.host}${window.location.pathname}mobile.html`;
		},
	...
	methods: {
		...
		testSoluton(solution) {
			const targetReportResult = this.audit.results.find(
		...
		refreshBrowserToSolution(solution) {
			...
			window.location = `${window.location.protocol}//${window.location.host}${window.location.pathname}?dl=${solution.deepLink}${addtionalParams}`;
		},
		toggleFullscreen() {
			const modalElements = document.getElementsByClassName("leopard-config-modal");
		...
		importSolution(newSolution) {
			const existingSolutionsWithId = this.config.solutions.findIndex(
				solution => solution.id === newSolution.id
			);

			const existingSolutionsWithName = this.config.solutions.findIndex(
		...
		readConfigFile(event) {
			...
			const self = this;
			reader.onload = function whenLoaded() {
		...
		refreshBrowser() {
			if (this.selectedSolution) {
				...
				window.location = `${window.location.protocol}//${window.location.host}${window.location.pathname}?dl=${this.selectedSolution.deepLink}${addtionalParams}`;
			} else {
		...
		cloneSolution() {
			...
			const clonedSolution = cloneObject(this.selectedSolution);
		...
		editSolutionAudit(solutionId) {
			const foundSolution = this.config.solutions.find(solution => solution.id === solutionId);
			...
			if (this.selectedSolution) {
				...
				if (this.config.solutions.length === 1) {
					...
					this.selectedSolution = cloneObject(
						this.config.solutions.find(function findActiveSolution(solution) {
		...
		ensureDefaultSolutionIsSet() {
			// this.config.activeSolution
			const foundActiveSolution = this.config.solutions.find(
				solution => solution.id === this.config.activeSolution
			);
