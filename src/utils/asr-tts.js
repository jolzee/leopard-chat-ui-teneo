    ...
    tts.initialize({
      ...
      lang:
        locale === "fr"
          ...
          : locale === "id"
          ? "id-ID"
          : String(locale).startsWith("en-us")
          ? "en-US"
          : "en-GB",
      debug: false
    });
  ...
  if (store.state.tts.tts !== null) {
    store.state.asr.asr = store.state.tts.tts.newDictation({
      ...
      onResult(text) {
        ...
        timeoutVar = setTimeout(function AbortRecognition() {
      ...
      onEnd() {
        ...
        if (store.getters.userInput) {
          ...
          asrCorrections.forEach(replacement => {
            const startingText = fixedUserInput;
            ...
            if (replacement[0].indexOf(".") > -1) {
              ...
              const search = replacement[0].replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&"); // escase any special characters
              const re = new RegExp(`\\b${search}\\b`, "gi");
          ...
          setTimeout(function signalReadyForSending() {
