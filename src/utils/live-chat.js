...
const logger = require("@/utils/logging").getLogger("live-chat.js"); // live chat
...
const md = require("markdown-it")({
	...
	typographer: true,
	quotes: "“”‘’"
}).use(iterator, "url_new_win", "link_open", function handleNewWindowLinks(tokens, idx) {
...
export default class LiveChat {
		...
		try {
			if (this.store.getters.enableLiveChat && window.leopardConfig.liveChat.licenseKey) {
				this.sdk = CustomerSDK.init({
					license: parseInt(window.leopardConfig.liveChat.licenseKey, 10),
				...
				this.sdk.auth.getToken().then(token => {
				...
				this.sdk.on("thread_summary", threadSummary => {
					...
					if ("lc2" in threadSummary.properties) {
						try {
							const secondsWait = threadSummary.properties.lc2.queue_waiting_time;
							const message = `Chat request sent to agent. You are number ${
								threadSummary.properties.lc2.queue_pos
							} in the queue. Average wait time is ${Math.floor(
								secondsWait / 60
							)} min ${`0${Math.floor(secondsWait % 60)}`.slice(-2)} seconds`;
	...
	startLiveChat() {
		if (!this.store.state.liveAgent.isLiveChat) {
			...
			const message = `You are talking to ${this.agent.name}.`;
