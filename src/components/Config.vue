  ...
  <v-layout>
    <v-dialog v-model="showModal" fullscreen hide-overlay transition="dialog-bottom-transition">
      ...
      <v-card>
        <v-toolbar fixed>
          <v-flex>
            <v-btn icon dark @click="refreshBrowser" color="green" :loading="refresh">
          ...
          <v-badge left overlap color="blue" class="mr-2">
            ...
            <v-icon large color="grey lighten-1">fa-cogs</v-icon>
          </v-badge>
          <v-chip v-if="config.activeSolution" disabled color="green" text-color="white" class="mr-4">
        ...
        <v-container class="mt-5 pt-5">
          <v-flex xs12 md10 offset-md1>
            ...
            <v-layout class="mb-2">
              <!-- help dialog -->
              <help-dialog :showDialog="helpDialog" :closeAction="closeHelpDialog"></help-dialog>
            ...
            <v-tooltip open-delay="600" bottom>
              <v-btn slot="activator" fab dark small @click="addSolution" color="green">
                <v-icon dark>add_circle</v-icon>
              </v-btn>
              <span>Create a new solution config</span>
            </v-tooltip>
            <v-tooltip open-delay="600" bottom>
              <v-btn slot="activator" fab dark small color="pink" @click="showUploadDialog">
                <v-icon dark>cloud_upload</v-icon>
              </v-btn>
              <span>Upload an individual or full solution config</span>
            </v-tooltip>
            <v-tooltip open-delay="600" bottom>
              <v-btn slot="activator" fab dark small color="indigo" @click="copyWholeConfigClipboard">
                <v-icon dark>fa-clipboard</v-icon>
              </v-btn>
              <span>Copy full solution config to clipboard</span>
            </v-tooltip>
            <v-tooltip open-delay="600" bottom>
              <v-btn slot="activator" fab dark small color="teal darken-3" @click="downloadSolutionConfig">
                <v-icon dark>fa-download</v-icon>
              </v-btn>
              <span>Download all solutions configs as a file</span>
            </v-tooltip>
            <v-tooltip open-delay="600" bottom>
              <v-btn slot="activator" fab dark small color="brown darken-3" @click="helpDialog = true">
            ...
            <v-dialog v-model="uploadDialog" scrollable persistent max-width="calc(1200px - 20vw)">

              <v-card>
                <v-card-title>
                  <h3>Upload Configurations
                    <v-progress-circular v-if="showProgressUpload" :size="30" :width="3" indeterminate color="purple"></v-progress-circular>
                  </h3>
                </v-card-title>
                <v-divider></v-divider>
                  ...
                  <v-layout row>
                    <v-flex xs12>
                      <div class="upload-btn ml-2 mb-2">
                        <input id="uploadFile" type="file" accept="text/*" name="uploadFile" @click="toggleLoading" @change="readConfigFile" />
                        <label for="uploadFile" class="v-btn primary upload-btn">
                          Upload
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="blue-grey lighten-5" light @click="closeUploadDialog">Close</v-btn>
                  <v-btn color="green" @click="saveUploadForm">Save
                    <v-icon right dark>fa-save</v-icon>
                  </v-btn>
                  <v-snackbar :timeout="globalSnackbarTimeout" v-model="globalSnackbar" absolute>
                    {{ globalSnackbarMessage }}
                  </v-snackbar>
                </v-card-actions>
              </v-card>
            ...
            <!-- Add Edit Dialog -->
            <v-dialog v-model="displayAddEditDialog" scrollable persistent max-width="calc(1200px - 10vw)">
              <v-card>
                <v-card-title>
                  <h3>{{ dialogTitle }}</h3>
                </v-card-title>
                <v-divider></v-divider>
                <v-card-text style="height: 90vh" id="add-edit">
                  <v-form ref="form">
                    <v-btn color="light-blue darken-1" href="https://fontawesome.com/icons?d=gallery&m=free" target="_blank">Font Awesome
                    </v-btn>
                    <v-btn color="light-blue darken-1" href="https://material.io/tools/icons/?icon=supervised_user_circle&style=baseline" target="_blank">Material
                    </v-btn>
                    <v-btn color="light-blue darken-1" href="https://htmlcolorcodes.com/color-names/" target="_blank">HTML Color Names
                    </v-btn>
                        ...
                        <v-flex xs4>
                          ...
                          <v-flex flex>
                            <v-layout row wrap>
                              ...
                              <v-flex d-flex>
                                <v-chip @click="setActiveColor('secondary')" :style="getBackGroundColor(solution.theme.secondary)" text-color="white">
                                  <v-avatar>
                                    <v-icon>invert_colors</v-icon>
                                  </v-avatar>
                                  secondary
                              ...
                              <v-flex d-flex>
                                <v-chip @click="setActiveColor('accent')" :style="getBackGroundColor(solution.theme.accent)" text-color="white">
                                  <v-avatar>
                                    <v-icon>invert_colors</v-icon>
                                  </v-avatar>
                                  accent
                              ...
                              <v-flex d-flex>
                                <v-chip @click="setActiveColor('error')" :style="getBackGroundColor(solution.theme.error)" text-color="white">
                                  <v-avatar>
                                    <v-icon>invert_colors</v-icon>
                                  </v-avatar>
                                  erorr
                              ...
                              <v-flex d-flex>
                                <v-chip @click="setActiveColor('info')" :style="getBackGroundColor(solution.theme.info)" text-color="white">
                                  <v-avatar>
                                    <v-icon>invert_colors</v-icon>
                                  </v-avatar>
                                  info
                              ...
                              <v-flex d-flex>
                                <v-chip @click="setActiveColor('success')" :style="getBackGroundColor(solution.theme.success)" text-color="white">
                                  <v-avatar>
                                    <v-icon>invert_colors</v-icon>
                                  </v-avatar>
                                  success
                              ...
                              <v-flex d-flex>
                                <v-chip @click="setActiveColor('warning')" :style="getBackGroundColor(solution.theme.warning)" text-color="white">
                                  <v-avatar>
                                    <v-icon>invert_colors</v-icon>
                                  </v-avatar>
                                  warning
                        ...
                        <v-flex xs8>
                          <v-layout>
                            <v-flex d-flex>
                              ...
                              <v-text-field v-model.trim="solution.theme.secondary" @click="setActiveColor('secondary')" validate-on-blur color="light-blue darken-1" :value="solution.theme.secondary" :tabindex="getTabIndex" label="secondary" :rules="[ruleMustHaveValue, ruleMustHaveColor]"></v-text-field>
                            </v-flex>
                          </v-layout>

                          <v-layout>
                            <v-flex d-flex>
                              <v-text-field v-model.trim="solution.theme.accent" @click="setActiveColor('accent')" validate-on-blur color="light-blue darken-1" :value="solution.theme.accent" :tabindex="getTabIndex" label="accent" :rules="[ruleMustHaveValue, ruleMustHaveColor]"></v-text-field>
                              <v-text-field v-model.trim="solution.theme.error" @click="setActiveColor('error')" validate-on-blur color="light-blue darken-1" :value="solution.theme.error" :tabindex="getTabIndex" label="error" :rules="[ruleMustHaveValue, ruleMustHaveColor]"></v-text-field>
                            </v-flex>
                          </v-layout>
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="blue-grey lighten-5" light @click="closeAddNewSolutionDialog">Close</v-btn>
                  <v-btn color="green" @click="saveForm">Save
                    <v-icon right dark>fa-save</v-icon>
                  </v-btn>
    ...
    <textarea id="copy-solution" readonly style="position:absolute;left:-9999px" v-text="getCurrentSelectedSolutionConfig"></textarea>
    <textarea id="copy-whole-config" readonly style="position:absolute;left:-9999px" v-text="getFullSolutionConfig"></textarea>
  </v-layout>
</template>
<style>
.select-options {
...
export default {
  ...
  computed: {
    ...
    sortedSolutions() {
      return this.config.solutions.sort(this.compareSolutions);
    },
    hasSolutions() {
